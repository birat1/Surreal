...
const UserProfileForm: React.FC<UserProfileFormProps> = ({
  ...
  const handleRemoveItem = (
    ...
    setFormData({
      ...formData,
      [field]: formData[field].filter((v) => v !== valueToRemove),
    });
  };

  const validateRequiredFields = () => {
    if (!formData.full_name.trim()) {
      toast.error('Full name cannot be left empty');
      return false;
    }
    if (!formData.age) {
      toast.error('Age cannot be left empty');
      return false;
    }
    if (!formData.username.trim()) {
      toast.error('Username cannot be left empty');
      return false;
    }
    if (!formData.university_year) {
      toast.error('University year cannot be left empty');
      return false;
    }
    if (!formData.fun_fact.trim()) {
      toast.error('Fun fact cannot be left empty');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url =
        mode === 'edit'
          ? 'http://localhost:8080/auth/edit-user-profile'
      ...
      if (profile_picture instanceof File) {
        ...
        profilePictureUrl = uploaded.profile_picture;
      ...
      if (mode === 'create' && userId && formData.username) {
        login(
          userId,
          formData.username,
